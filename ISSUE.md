...
The project documents end-to-end encryption as a core property. The [README](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/README.md#L8) states "Use Claude Code or Codex from anywhere with end-to-end encryption," and the [server README](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-server/README.md#L11) claims "Zero Knowledge - The server stores encrypted data but has no ability to decrypt it."

Session data (messages, code, artifacts) is encrypted client-side with the user's key, consistent with these claims. Vendor tokens from `happy connect` are not — they are encrypted with a server-held `HANDY_MASTER_SECRET` via a `KeyTree` from `privacy-kit`. The project's own [encryption documentation](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/docs/encryption.md#L518) acknowledges this: "These are encrypted with a server-only KeyTree derived from `HANDY_MASTER_SECRET` and are not end-to-end encrypted."

This means the server can decrypt and access these tokens at any time. It exposes endpoints that [decrypt and return individual tokens](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-server/sources/app/api/routes/connectRoutes.ts#L269-L292) or [all vendor tokens at once](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-server/sources/app/api/routes/connectRoutes.ts#L312-L332) in plaintext. This distinction is not communicated to users — the [CLI help text](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-cli/src/commands/connect.ts#L61-L63) describes the feature only as: "The connect command allows you to securely store your AI vendor API keys in Happy cloud."
...
The inconsistency between the three vendors makes it hard to see what unified feature could be built on these tokens. The [Gemini flow](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-cli/src/commands/connect/authenticateGemini.ts#L22-L26) grants root access to all of GCP. The [Claude flow](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-cli/src/commands/connect/authenticateClaude.ts#L18) grants inference-only access (`user:inference`). The [Codex flow](https://github.com/slopus/happy/blob/d343330c86ab966969aecd82be4aecbad7ec4238/packages/happy-cli/src/commands/connect/authenticateCodex.ts#L253) grants only identity scopes (`openid profile email offline_access`) — no API access at all. If the intent were to proxy inference requests, all three would need inference-level access; if the intent were identity federation, none would need `cloud-platform`. The current state looks like three independently written integrations with no shared design for what the tokens are for.
